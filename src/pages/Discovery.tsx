...
const DEMO_DISCOVERY_RECORD: Record<string, unknown> = {
...
const useDemoDiscoveryDataLoader = ({
  ...
  const filteredData = useMemo(() => {
    if (!normalizedSearch) return [DEMO_DISCOVERY_RECORD];
    const haystack = searchableFields
      .map((field) => toSearchableString(getFieldValue(DEMO_DISCOVERY_RECORD, field)))
      .join(' ')
      .toLowerCase();
    return haystack.includes(normalizedSearch) ? [DEMO_DISCOVERY_RECORD] : [];
...
registerDiscoveryDataLoader(
  ...
  useDemoDiscoveryDataLoader as any,
);
